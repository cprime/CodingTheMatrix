...
def equal(u,v):
    ...
    assert u.D == v.D
    r = True
    for p in u.f: r&=(u.f[p]==v.f[p] if p in v.f else u.f[p]==0)
    for p in v.f: r&=(u.f[p]==v.f[p] if p in u.f else v.f[p]==0)
    return r

def add(u,v):
    "Returns the sum of the two vectors"
    assert u.D == v.D
    return Vec(u.D,{d:((u[d]+v[d]) if (d in u.f and d in v.f) else (u[d] if d in u.f else (v[d] if d in v.f else 0))) for d in u.D})

def dot(u,v):
    "Returns the dot product of the two vectors"
    assert u.D == v.D
    return sum([((u[d]*v[d]) if (d in u.f and d in v.f) else 0) for d in u.D])
